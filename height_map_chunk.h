...
public:
    ...
    HeightMapChunk(Spatial *p_parent, Point2i p_cell_pos, Ref<Material> p_material, Ref<Material> decel_material);
	~HeightMapChunk();

	void create(Point2i pos, Ref<Material> material);
	void set_mesh(Ref<Mesh> mesh);
    void set_decel_mesh(Ref<Mesh> mesh);
	void clear();
	void set_material(Ref<Material> material);
    void set_decel_material(Ref<Material> material);
	void enter_world(World &world);
	void exit_world();
	void parent_transform_changed(const Transform &parent_transform);
	void set_visible(bool visible);
    bool is_visible() const { return _visible; }
    void set_decel_visible(bool decel_visible);
